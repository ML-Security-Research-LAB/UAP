...
class CDD(object):
    def __init__(self, kernel_num, kernel_mul, 
                 num_classes, device, num_layers=1, intra_only=False, **kwargs):
    ...
    def forward(self, source, target, nums_S, nums_T):
        ...
        if not self.intra_only:
            ...
            inter_mask = ((torch.ones([num_classes, num_classes]) \
                      - torch.eye(num_classes)).bool()).to(self.device)
...
class CDDLoss(nn.Module):

    def __init__(self, num_classes=7, device='cuda:0'):
        super().__init__()
        KERNEL_NUM = (5, 5)
        KERNEL_MUL = (2, 2)
        self.cdd = CDD(KERNEL_NUM, KERNEL_MUL, num_classes, device=device)
        print('Using CDD Loss')
        

    def forward(self, f, y, means, covs):
        # Initialize lists to store generated source features and target features from 'f'
        source_features, target_features = [], []

        # Identify unique classes and their respective counts in 'y'
        unique_classes, class_counts = torch.unique(y, return_counts=True)

        # Loop through each unique class to generate features and gather class-wise features
        for c, count in zip(unique_classes, class_counts):
            # Retrieve mean and covariance for the current class from input arguments
            mu, cov = means[c], covs[c]

            # Create a multivariate Gaussian distribution using the mean and covariance
            dist = torch.distributions.MultivariateNormal(mu, cov)

            # Sample features from the Gaussian distribution
            sampled_features = dist.sample((count.item(),))

            # Store the sampled features for the source domain
            source_features.append(sampled_features)

            # Collect features from 'f' corresponding to the current class
            classwise_target_f = f[y == c]

            # Store the collected features for the target domain
            target_features.append(classwise_target_f)

        # Concatenate features across all classes for source and target domains
        source_features = torch.cat(source_features, dim=0)
        target_features = torch.cat(target_features, dim=0)

        # Compute the contrastive domain discrepancy (CDD) loss
        loss = self.cdd.forward([source_features], [target_features], class_counts, class_counts)
...
if __name__ == '__main__':
    ...
    mu = torch.randn(2, 20).to(device)
    cov = torch.eye(20).to(device)

    dist1, dist2 = torch.distributions.MultivariateNormal(mu[0], cov), torch.distributions.MultivariateNormal(mu[1], cov)
    source = torch.cat((dist1.sample((100,)), dist2.sample((100,))))
    target = torch.cat((dist2.sample((100,)), dist1.sample((100,))))
    # print(source.shape, target.shape)

    cdd = CDD(KERNEL_NUM, KERNEL_MUL, num_classes, device)
    loss = cdd.forward([source], [target], [100, 100], [100, 100])
    print(loss)